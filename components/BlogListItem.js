...
function BlogListItem({ post }) {
    ...
    const postDate = formatDate(postDate)
    return (
        <article class="py-8">
            <TitleLink href={post.path}>
                {postTitle}
            </TitleLink>
            <div class="text-sm text-gray-600 leading-normal mb-2">{postDate}</div>
            <p class="text-black mb-6 leading-loose text-base">{post.excerpt}</p>
            <Link href={post.path}>
                <a class="group inline-flex items-center text-sm text-blue no-underline uppercase tracking-wide">
