...
function BlogListItem({ post }) {
    ...
    return (
        <article className="py-8">
            ...
            <time className="block text-sm text-gray-600 dark:text-gray-500 leading-normal mt-4 mb-2">{post.frontmatter.date}</time>
            <p className="text-black dark:text-gray-200 mb-6 leading-loose text-base lg:text-lg">{post.frontmatter.excerpt}</p>
            <Link href={href} className="group inline-flex font-mono items-center text-sm text-blue dark:text-blue-400 no-underline uppercase tracking-wide">
