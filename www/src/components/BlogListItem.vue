<template>
  <article class="py-8">
    ...
    <div class="text-sm text-gray-600 leading-normal mb-2">{{ post.date | formatDate }}</div>
    <p class="text-black mb-6 leading-loose text-base">{{ post.excerpt }}</p>
    <g-link
      class="group inline-flex items-center text-sm text-blue no-underline uppercase tracking-wide"
      :to="post.path">
