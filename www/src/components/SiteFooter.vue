<template lang="html">
  <footer class="text-center relative">
      <div class="flex items-center justify-center text-hot-pink">
          <span class="w-40 inline-block">
                <svg class="w-full" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 42.31"><path d="M80 40.67a37.84 37.84 0 00-.61-8c-26-1.26-51.87-3.83-77.9-3.91A35.92 35.92 0 000 38v3c26.7.89 79.94 1.28 79.94 1.28s.06-1.06.06-1.61zm-1.66-12.32a41.71 41.71 0 00-2-5.23l-1.2-.12q-17.85-1.83-35.77-2.77h.12c-11.37-.45-22.74-.69-34.11-.58a41.29 41.29 0 00-3.33 7.17c25.65 0 51.17 2.32 76.77 3.45-.09-.65-.29-1.27-.48-1.92zm-3.67-8.45a38.31 38.31 0 00-6.28-8.1c-17-1.35-33.91-3.57-50.91-4.9A39 39 0 006.73 17.61c6.57 0 13.13.12 19.7.26 5.4.24 10.8.36 16.19.73 3.56.24 7.13.46 10.7.71h-.11c4.28.36 8.55.78 12.83 1.15h-.11.25-.11.81c2.88.28 5.75.59 8.62.88-.28-.43-.56-.94-.83-1.44zM66.32 9.81a40.18 40.18 0 00-47.58-3.73C35 7.43 51.16 9.49 67.4 10.85z" fill="currentColor"/></svg>
          </span>
      </div>
      <span class="absolute top-0 transform -rotate-22 translate-y-full mt-0 ml-20 w-4 text-yellow">
          <span class="absolute bottom-0 left-0 pt-4 transform rotate-22 -translate-x-1/2 translate-y-full w-48 text-sm font-mono text-yellow">this is a sunset</span>
          <svg xmlns="http://www.w3.org/2000/svg" class="transform rotate-180 h-full w-full" viewBox="0 0 11 75.7"><path d="M6.21 29.25a3.74 3.74 0 010-.66c0-.22-.09-.53-.1-.74 0-.41-.12-.81-.13-1.23 0-1.15-.07-2.3-.09-3.46v-.6H6a3.85 3.85 0 000-.48 1.28 1.28 0 010-.33s-.06-.23-.06-.27a.94.94 0 01-.09-.18l-2.08.05c0 .23.08.47.1.7V23l.07 2.84c0 1.05-.31 2.11-.31 3.12a1.55 1.55 0 00.2.77c.15.23.11.3.22.38a.93.93 0 00.6.12c.66-.06 1.63-.47 1.56-.98zm-.35-9.17v-.28a.26.26 0 000-.17 19.57 19.57 0 00-.12-2 2.42 2.42 0 010-.71 9.24 9.24 0 00.07-1.28c0-.2-.1-.33 0-.61a2.23 2.23 0 00.08-.45c0-.21-.06-.32-.08-.53 0-.48-.08-.95-.1-1.43a1.19 1.19 0 01.06-.37.83.83 0 000-.58.72.72 0 01-.11-.53 3.86 3.86 0 000-1V6.92a12 12 0 00.08-1.45A27.87 27.87 0 015.81 2c0-.52-.28-1.76-.62-1.91S4.74.1 4.54 0c-.47-.15-1.26.63-1.25 1.33l.11 2.22v2.31c0 .61.23 1.22.21 1.81v1.8a2.64 2.64 0 000 .93c0 .68 0 1.38-.06 2.07a11.2 11.2 0 00.1 1.3c.05 1.19 0 2.38.13 3.56v.89a3.07 3.07 0 01-.05 1.09v.06a1.14 1.14 0 000 .37 2.48 2.48 0 010 .72c.66 0 1.46.06 2.13 0a.45.45 0 000-.2.86.86 0 010-.18z" fill="currentColor"/><path d="M5.88 26.38a5.4 5.4 0 010-.87c0-.38.08-1.11.06-1.49s0-.61 0-.87v-.44c0-.47-.07-.95-.1-1.42a1.67 1.67 0 01.06-.38.71.71 0 000-.57.72.72 0 01-.11-.53 3.16 3.16 0 000-1v-.79a2.93 2.93 0 01-.1-.65c-.53.07-1.11.11-1.66.15a1.51 1.51 0 01-.17.52.74.74 0 010 .12c-.13.3 0 .61 0 .92v2.07a11.37 11.37 0 00.09 1.31v3.84a3.11 3.11 0 011.93.08zM5.31 45.1a.73.73 0 01-.78-.1 1.14 1.14 0 01-.33-1.06 14.11 14.11 0 00-.09-3.11c0-1.12 0-2.29-.11-3.41a9.87 9.87 0 00-.28-2.72 1.57 1.57 0 01.55-1.95 1.3 1.3 0 011.07-.11c.7.23.79 1.24.83 1.86s-.29 1.09-.27 1.74c0 .32-.06.64-.07 1 0 1.13 0 2.27-.07 3.41 0 .53-.17 1-.23 1.58s.17 1.06.14 1.59c-.02.27-.01 1.1-.36 1.28zM3.84 49c0-.37 0-1.49.55-1.53a1.1 1.1 0 01.68.27 8.68 8.68 0 01.9.7 1.18 1.18 0 01.34.46c.17.49-.09 1.06-.09 1.56A19.31 19.31 0 015.69 53c-.14.95.15 1.76-.66 2.27-.23-.24-.6-.56-.64-.91-.18-1.7-.39-3.36-.53-5.15a2.46 2.46 0 01-.02-.21zm.35 11.35a1.73 1.73 0 01.22-1c.46-.76 1.81-.94 1.93.15a6.46 6.46 0 01-.22 1.79c-.06.43-.38 1.84-.94 1.44a4.07 4.07 0 01-.93-2 2.83 2.83 0 01-.06-.38zm1.44 8.46c-1 .15-1.77-1.55-1.19-2.32a1.36 1.36 0 012-.07c.39.64-.03 2.21-.81 2.39z" fill="currentColor"/><path d="M5.92 75.7c1.47-.07 5.66-9.11 5-9.7S.79 65.68 0 66.51c-.43.49 4.61 9.25 5.92 9.19z" fill="currentColor"/></svg>
      </span>
      <nav class="pt-20 pb-8 bg-gradient-to-b from-hot-pink to-purple-700 -mt-1">
        <HorizontalPad>
            <div class="relative mb-40">
                <span class="absolute top-0 transform -translate-x-full -translate-y-full -ml-20 -mt-4 w-4 text-yellow">
                    <span class="absolute top-0 left-0 -mt-3 transform -rotate-2 -translate-x-1/2 -translate-y-full w-48 text-sm font-mono text-yellow">these are some links</span>
                    <svg class="transform -rotate-4 h-full w-full" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 11 72.84"><path d="M5.9 12.46c-.07.2-.28.38-.19.6.17.43-.21.53-.47.61a1.63 1.63 0 01-1.55-.17c-.47-.32-.26-.57-.21-.95a1.27 1.27 0 011.18-1.16 1.55 1.55 0 01.69.29c.13.1.31.14.42.26a.65.65 0 01.14.29.44.44 0 01-.01.23zm-.6 4.78c.13.09.57.23.64.46.55 1.39-.87 1.69-1.38 1.6s-1.07-.29-1.1-.51c-.16-.99.36-1.43 1.84-1.55zM3.2 1.38a.93.93 0 010-.13 5 5 0 00.12-.74c.05-.24.16-.27.45-.36A1.38 1.38 0 015 .13c.35.13.48.27.69.73s-.22 1.02-1.07 1.38a.76.76 0 01-.47 0c-.22-.12-.42-.27-.63-.41s-.37-.25-.32-.45zm2.72 5.79a1.82 1.82 0 01-1.68.87c-.47-.24-1-.75-1-1.14s.62-.85 1-1.27c1.42-.05 1.76.98 1.68 1.54zm.33 28.23c-.07.21-.27.39-.18.61.17.43-.21.52-.48.6a1.64 1.64 0 01-1.54-.16c-.47-.33-.26-.57-.21-1A1.27 1.27 0 015 34.33a1.42 1.42 0 01.69.3 2.05 2.05 0 01.43.26.65.65 0 01.14.29.33.33 0 01-.01.22zm-.59 4.84c.12.08.57.23.63.45.55 1.39-.87 1.7-1.38 1.6s-1.06-.29-1.1-.5c-.16-1.04.37-1.49 1.85-1.55zM3.55 24.32a1 1 0 000-.13 4.78 4.78 0 00.13-.74c0-.23.15-.27.44-.36a1.42 1.42 0 011.21 0c.35.14.48.28.69.74s-.24 1-1.09 1.32a.66.66 0 01-.46 0c-.22-.12-.43-.27-.63-.4s-.33-.22-.29-.43zm2.72 5.8a1.84 1.84 0 01-1.67.87c-.47-.25-1-.76-1-1.15s.63-.84 1-1.26c1.4-.05 1.77.98 1.67 1.54zm.34 28.23c-.07.2-.28.38-.19.6.17.43-.21.53-.47.61a1.63 1.63 0 01-1.55-.17c-.47-.32-.26-.57-.21-.95a1.27 1.27 0 011.18-1.2 1.55 1.55 0 01.69.29c.13.1.31.14.42.26a.65.65 0 01.14.29.44.44 0 01-.01.27zM6 63.17c.13.09.57.23.64.46.55 1.39-.87 1.69-1.38 1.6s-1.07-.29-1.1-.51C4 63.69 4.53 63.24 6 63.17zM3.91 47.24a.93.93 0 010-.13 5 5 0 00.12-.74c0-.24.16-.27.45-.36a1.38 1.38 0 011.21 0c.35.13.48.27.69.73s-.24 1-1.09 1.33a.73.73 0 01-.46 0 7 7 0 01-.64-.41c-.19-.16-.33-.19-.28-.42zm2.72 5.82a1.82 1.82 0 01-1.63.87c-.47-.24-1-.75-1-1.14s.62-.85 1-1.27c1.37-.05 1.72.98 1.63 1.54z" fill="currentColor"/><path d="M5.92 72.84c1.47-.07 5.66-9.11 5-9.7S.79 62.82 0 63.65c-.43.51 4.61 9.25 5.92 9.19z" fill="currentColor"/></svg>
                </span>
            </div>
        </HorizontalPad>
        <FooterLink
            v-for="(link, index) in settings"
            :key="index"
            :index="index"
            :title="link.title"
            :to="link.to">
        </FooterLink>
      </nav>
  </footer>
</template>

<script>
import HorizontalPad from '@/components/HorizontalPad'
import FooterLink from '@/components/FooterLink'
import settings from '@/data/settings'

export default {
  name: 'SiteFooter',
  components: {
    HorizontalPad,
    FooterLink,
  },
  created() {
    this.settings = settings.footerLinks
  }
}
</script>
